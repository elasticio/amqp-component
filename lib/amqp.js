...
class AMQPClient {
  constructor(cfg, context) {
    ...
    this.reconnectTimeOut = cfg.reconnectTimeOut ? Number(cfg.reconnectTimeOut) : null || RECONNECT_TIMEOUT;
    this.reconnectAttempts = cfg.reconnectAttempts ? Number(cfg.reconnectAttempts) : null || RECONNECT_ATTEMPTS;
  }
  ...
  init(confirmChannel = true, assertExchangeOptions) {
        ...
        setup: async (channel) => {
          try {
            ...
            if (!confirmChannel) {
              ...
              await channel.assertQueue(this.queueName, { exclusive: false, durable: false, autoDelete: true });
